...
extension String {
    ...
    /// Words that are generally ignored during grading/scoring
    ...
    /// Returns a list of words stripped of punctuation and lowercased.
    /// Used for strict grading where every word counts.
    var allWords: [String] {
        let clean = self
            .lowercased()
    ...
    /// Returns a list of "gradable" words (lowercased, punctuation removed, common words ignored).
    /// Used for scoring to give the user a break on small words.
    ...
    /// Generates a colored string by matching this string against spoken words.
    func colored(matching spokenWords: [String], font: UIFont) -> NSAttributedString {
        ...
        // Default Style: Gray (Unread) with DYNAMIC FONT
        ...
        for (i, match) in matches.enumerated() {
            ...
            // Check if this word is "ignorable" (skip coloring it Red if missed)
//            let isIgnorable = String.ignorableWords.contains(cleanWord)
            ...
            if foundMatch {
                ...
                if spokenIndex < spokenWords.count {
                    ...
                    if !isNextWordMatch {
                        // Only mark RED if it's not an ignorable word
                        // (Optional tweak: keep your original logic if you want 'a' to turn red)
    ...
    /// Determines if this string matches the target using phonetic rules.
