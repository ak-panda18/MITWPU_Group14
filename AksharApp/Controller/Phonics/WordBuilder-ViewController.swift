...
class WordBuilder_ViewController: UIViewController, ExerciseReceivesCover, ExerciseResumable, ExerciseProgressReporting {
    ...
    private func setupInitialization() {
        ...
        phonicsSession = PhonicsSessionData(
            id: UUID(), date: Date(), childId: "default_child",
            exerciseType: "word_builder", correctCount: 0,
            totalAttempts: 0, startTime: Date(), endTime: nil
        )

        let storageKey = "word_builder_cycle"
        questionCycle = ExerciseCycleStore.load(key: storageKey) ??
                         RandomizedQuestionCycle(count: questions.count, startPointer: startingIndex ?? 0)
        
        questionIndex = questionCycle.currentIndex()
    }
    ...
    @IBAction func submitTapped(_ sender: UIButton) {
        ...
        if assembled.lowercased() == q.correct.lowercased() {
            ...
            feedbackLabel.text = "Correct!"; feedbackLabel.textColor = .systemGreen
            submitButton.setTitle("Next", for: .normal)
            isAnswerLocked = true
            triggerConfetti()
        } else {
            feedbackLabel.text = "Try again!"; feedbackLabel.textColor = .systemRed
        }
    ...
    private func nextQuestion() {
        ...
        questionIndex = questionCycle.currentIndex()
        ExerciseCycleStore.save(questionCycle, key: "word_builder_cycle")
        submitButton.setTitle("Submit", for: .normal)
        loadQuestion()
    }
