...
class FluencyDrillsViewController: UIViewController,
    ...
    func styleSubmitButton() {
        ...
        nextButton.backgroundColor =
            UIColor(red: 117/255, green: 80/255, blue: 50/255, alpha: 1.0)
    ...
    func startTimer() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { t in
            ...
            self.timerLabel.text =
                "\(self.secondsRemaining) seconds remaining"
            self.progressView.progress =
                Float(self.secondsRemaining) / Float(self.totalSeconds)
    ...
    func startListening() {
        ...
        recognitionTask =
            speechRecognizer?.recognitionTask(with: recognitionRequest) {
                ...
                guard
                    let result = result,
                    self.items.indices.contains(self.currentIndex),
    ...
    // MARK: - ROUND END UI (ALWAYS SHOWN)
    func showRoundEndUI() {
        scoreLabel.text =
            "You pronounced \(correctCount) words correctly!"
    ...
    // MARK: - Session End (SAVE ONCE)
        func endSession() {
            timer?.invalidate()
            audioEngine.stop()
            if !hasSavedSession, var session = phonicsSession {
                session.endTime = Date()
                session.correctCount = self.correctCount
                phonicsSession = session
    ...
    @IBAction func homeButtonTapped(_ sender: Any) {
        endSession()
        ExerciseCycleStore.clear(key: "fluency_cycle")
        goHomeFromPhonics()
    }
}
