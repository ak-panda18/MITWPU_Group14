...
private extension ImageLabelReadingViewController {
    func navigateToCheckpoint(
        ...
        guard let storyboard = storyboard,
              let cpVC = storyboard.instantiateViewController(
                withIdentifier: "CheckpointVC"
              ) as? CheckpointViewController else {
            return false
        }
        ...
        cpVC.readingSession = readingSession
    ...
    func isCurrentCheckpointCompleted() -> Bool {
            ...
            let response = CheckpointsResponse()
            guard let item = response.item(for: story.id, pageNumber: currentPage.pageNumber) else { return false }
            
            let storyID = story.title.replacingOccurrences(of: " ", with: "")
            let cleanText = item.text
                .components(separatedBy: CharacterSet.alphanumerics.inverted)
                .joined()
            let checkpointID = String(cleanText.prefix(50))
            let key = "CheckpointCompleted_\(storyID)_\(checkpointID)"
    ...
    func updateRetakeButtonVisibility() {
            ...
            retakeButton.isHidden = !UserDefaults.standard.bool(forKey: key)
        }
